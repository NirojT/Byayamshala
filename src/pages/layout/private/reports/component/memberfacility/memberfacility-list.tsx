...
import MemberFacilityFilters from "./component/memberfacility-filters";

const MemberFacilityList: React.FC = () => {
  const navigate = useNavigate();

  const { currentSystemUser } = useSystemUserStore();
  const { memberFacilities, inDepthReport } = useListMemberFacilityStore();
            ...
            {!inDepthReport && (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody>
                    {Array.isArray(memberFacilities) &&
                    memberFacilities?.length > 0 ? (
                      memberFacilities.map((data, index) => (
                        <React.Fragment key={data?.id ?? index}>
                          ...
                          >
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-200">
                              {data?.facilityName} ({data?.memberName})
                            </td>
                          ...
                          {data?.facilities
                            ...
                            <td className="px-5 py-3 whitespace-nowrap text-gray-600 max-w-[150px] overflow-hidden text-ellipsis">
                              <span
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  alert(data?.description); // Or open a modal
                                }}
                              >
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-3 border-t border-gray-100 pt-2">
                    <div className="flex items-center gap-1">
                      ...
                      <span>Start: {data?.facilityStartDate}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <BsCalendarDate className="text-gray-400" />
                      <span>End: {data?.facilityEndDate}</span>
                    </div>
                  </div>
