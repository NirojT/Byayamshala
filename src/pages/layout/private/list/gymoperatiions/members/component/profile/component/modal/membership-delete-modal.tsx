...
const MembershipDeleteModal = () => {
  ...
  const { deleteMembership, openDelete, setOpenDelete, data, setData } =
    useListMembershipStore();

  const handleDelete = async () => {
    if (!data) return;
    const res = await deleteMembership(data?.id);
    setToasterData({
      open: true,
      message: res.message,
      ...
      <AlertDialogContent className="sm:max-w-[450px]">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="mt-2 text-sm text-gray-600">
            This will permanently delete the {data?.planName} Membership and all
            related transaction and entries.
