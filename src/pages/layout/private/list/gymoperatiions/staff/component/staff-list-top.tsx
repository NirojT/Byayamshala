...
import StaffModal from "./modal/staff-list-modal";
import { Power, Trash2 } from "lucide-react";

const StaffListTop = () => {
  const { setBulkTask, openModal, setOpenModal, filteredData } =
    useListStaffStore();
  ...
  return (
    <div className="z-50 mt-12 ml-4 mr-4">
      {isSomeSelected && (
        <Typography
          variant="h6"
          ...
          gutterBottom
        >
          <p className="sm:font-extrabold sm:text-lg font-poppins font-bold text-sm text-center">
     ...
     sm:p-2 rounded-lg hover:text-red-600 transition-colors flex items-center justify-center
      ...
      {isSomeSelected && openModal && <StaffModal />}
    </div>
  );
};

export default StaffListTop;
