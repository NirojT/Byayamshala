...
import { viewOperations } from "../../add/view-operations";
    ...
    <div className="flex flex-col w-full">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {viewOperations.map((operation) => (
          <button
            key={operation.id}
            onClick={() => navigate(`/${PrivateRoute}/list/gym/operation/${operation.name}`)}
            className="w-full"
          >
            <Card
...
const getDescription = (category: string) => {
  switch (category.toLowerCase()) {
    case 'members':
      return 'View all gym members';
    case 'visitors':
      return 'View visitor records';
    case 'trainers':
      return 'View all trainers and instructors';
    case 'plans':
      return 'View membership plans';
    case 'facilities':
      return 'View gym facilities';
    case 'task':
      return 'View assigned tasks';
    case 'supplier':
      return 'View supplier information';
    default:
